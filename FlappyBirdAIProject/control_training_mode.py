...
def run(config_path):
    ...
    pickle.dump(best, open(os.path.join(sys.path[0], 'ControlBirds/control_bird.pkl'), 'wb'))
    
    results = pd.DataFrame(results)
    
    results.to_csv(os.path.join(sys.path[0], 'ControlBirdData/control_bird_data.csv'), mode='a', index = False, header = False)
