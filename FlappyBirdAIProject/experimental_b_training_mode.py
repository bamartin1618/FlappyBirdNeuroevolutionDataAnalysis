...
pygame.display.set_caption('Flappy Bird AI Experimental B')
...
def run(config_path):
    ...
    pickle.dump(best, open(os.path.join(sys.path[0], 'ExperimentalBirdsB/experimental_bird_b.pkl'), 'wb'))
    
    results = pd.DataFrame(results)
    
    results.to_csv(os.path.join(sys.path[0], f'ExperimentalBirdBData/experimental_bird_b_data.csv'), mode = 'a', index = False, header = False)
...
if __name__ == '__main__':
    ...
    for index in range(6):
        processes.append(multiprocessing.Process(target = run, args = [config_path]))
